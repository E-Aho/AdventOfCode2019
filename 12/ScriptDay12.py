...
# Part 1
def find_sys_energy(step_lim):
    moons = get_moons(get_starting_coordinates())
    for step in range(1, step_lim + 1):
    ...
    return sum(get_TE(moon) for moon in moons)
...
def get_cycle_length(axis, moons):
    """"Gets number of steps until all planets returns to starting position in a given axis"""
    step = 0
    original_pos = list(map(int, [cords[axis] for cords in get_starting_coordinates()]))
    while True:
        step += 1
        moon_pairs = combinations(moons, 2)
        for pair in moon_pairs:
...
def get_time_to_repeat():
    ...
    return get_lcm(axis_cycles)


if __name__ == '__main__':

    sys_energy = find_sys_energy(1000)  # 9999. Yes literally.
    print('\nPart 1: \nTotal energy of system after 1000 steps:', sys_energy, '\n')
    cycle_length = get_time_to_repeat()  # 282399002133976
    print(f'Part 2:\nNumber of steps to return to start: {cycle_length}')
