...
def rotate(cur_dir, val):  # 0 -> turn left(counterclockwise), 1 -> turn right(clockwise)
    """returns rotated direction (left or right) based on val being 0 or 1"""
    if val == 0:
...
class PaintRobot:
    def __init__(self):
        self.computer = OpcodeComp(initial_state)
